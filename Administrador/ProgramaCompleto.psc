Algoritmo Unificar_proyecto
	// Acá nuestro codigo
		...
		Definir administradores Como caracter
		Dimension administradores[20,20]
		//los voy a usar para precargarlo
		administradores[1,1] <- "1001"//su id
		administradores[1,2] <- "jesucristonuestrosalvador"//contraseña
		administradores[1,3] <- "Martin"//nombre
		administradores[1,4] <- "Rodriguez"//apellido
		...
		//son nombres para probar y no ir poniendo uno en uno
		nombreTrabajador[1] <- "Pepito gamer"
		apellidoTrabajador[1] <- "Suarez"
		
		nombreTrabajador[2] <- "Lionel"
		apellidoTrabajador[2] <- "Messi"
		
		nombreTrabajador[3] <- "Carlitos"
		apellidoTrabajador[3] <- "Bala"
		
		nombreTrabajador[4] <- "Martina"
		apellidoTrabajador[4] <- "Gomez"
		...
		Repetir 
			...
			Segun num Hacer
				...
				3:
					Administracion(tipoEmergencia, provincia, localidad, descripcion, nivel, estado, idTrabajadorAsignado, cantMisiones, nombreTrabajador, apellidoTrabajador,administradores);
				De Otro Modo:
...
SubProceso Administracion(tipoEmergencia, provincia, localidad, descripcion, nivel, estado, idTrabajadorAsignado, cantMisiones, nombreTrabajador, apellidoTrabajador,administradores)
	
	Definir opcion Como Entero
	Definir id,asegurar Como Logico
	Limpiar Pantalla
	Verificacion(administradores,id,asegurar)
	si(asegurar=Verdadero)
		Repetir
			Limpiar Pantalla
			Escribir "======================================"
			...
			Escribir "3. Asignar mision"
			Escribir "4. Quitar mision"
			Escribir "5. Salir"
			Escribir "======================================"
...
SubProceso Verificacion(administradores,id Por Referencia,asegurar Por Referencia)
	...
	si(intentos<>5 y id<>Falso)
		intentos<-0
		Repetir
			Escribir "Ingrese su contraseña"
			leer contra
			Para i<-1 Hasta 3 Con Paso 1 Hacer
				si(administradores(i,2)=contra)
					asegurar<-Verdadero
				FinSi
			Fin Para
			si(asegurar<>Verdadero)
